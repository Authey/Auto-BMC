...
Std_ReturnType Rte_Call_LightActuator_frontLightActuator_DigitalLight_Write(/*IN*/DigitalLevel Level) {
    return Rte_Call_IoHwAb_ioHwAb_Digital_FrontLight_Write(Level);
}

/** ------ InteriorLightStatus */
/*lint -e621 MISRA:OTHER:Ignore misidentified symbol clash [MISRA 2012 Rule 5.5, required]*/
Std_ReturnType Rte_Read_LightActuator_frontLightActuator_InteriorLightStatus_status(/*OUT*/LightStatusImpl * value) {
    LightStatusImpl a = 100;
    value = &a;
    Std_ReturnType retVal = RTE_E_OK;

    /* --- Receiver (lightManager_FrontLightStatus_to_frontLightActuator_InteriorLightStatus) */
    {
        SYS_CALL_AtomicCopy32(*value, Rte_Buffer_frontLightActuator_InteriorLightStatus_status);
    }
